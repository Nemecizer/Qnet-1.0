...
// auxiliary-window contract for free: the `<app>.aux.` identifier ⌘W keys
...
struct DSDismissAction {
    ...
    static let unclaimed = DSDismissAction()

    @MainActor func callAsFunction() { perform() }
}

private struct DSDismissKey: EnvironmentKey {
    static let defaultValue = DSDismissAction.unclaimed
}

extension EnvironmentValues {
    /// Dismiss this form — closing the hosting panel window, or dismissing
    /// the sheet, whichever it turns out to be. See `DSDismissAction`.
    var qnetDismiss: DSDismissAction {
        get { self[DSDismissKey.self] }
        set { self[DSDismissKey.self] = newValue }
    }
}
...
private struct DSDismissBridge<Content: View>: View {
    ...
    var body: some View {
        if claimed.isClaimed {
            ...
            content.environment(\.qnetDismiss, DSDismissAction { dismiss() })
        }
    }
}

extension View {
    /// See `DSDismissBridge`. `DSSheet` applies this; nothing else needs to.
    func qnetDismissBridge() -> some View {
        DSDismissBridge(content: self)
    }
}
...
enum DSPanelWindow {
    ...
    static func frameKey(for id: String) -> String { GUIKitConfig.frameKey(for: "Panel.\(id)") }
    ...
    static func present<Root: View>(
        ...
        let window = AuxiliaryWindow.make(
            ...
            root(ref)
                ...
                // `\.qnetDismiss` closes THIS window (see DSDismissAction).
                .environment(\.qnetDismiss, DSDismissAction { ref.close() })
        }
