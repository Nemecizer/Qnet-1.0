...
//          `DSBadge(dot:)` takes a `dotSymbol:`; `SeriesChip` swaps its
...
//                       / `dotRing:`), 20-pt
//   SeriesChip           customer-class swatch + label (also the rows of class menus)
...
enum DS {
    ...
    enum A11y {
        ...
        /// shape or glyph companion (DSBadge `dot:`, SeriesChip swatch).
    ...
    enum Glossary {
        // A glossary entry is the definition behind a jargon-only label. Any
        // DS control that takes `glossary:` grows a `?` in a reserved trailing
        // column — reserved so that fields WITH and WITHOUT a definition keep
        // the same trailing edge, and a form does not look ragged.
        //
        // The rule the lint enforces: every entry here must be referenced by at
        // least one control, and `all` must name every entry. An orphaned
        // definition is worse than a missing one, because it reads as coverage
        // that is not there. That is why this kit ships four illustrative
        // entries rather than a domain vocabulary — REPLACE them with your own.
        //
        // Write each one as: symbol/term — plain-language meaning, then the
        // detail that actually helps. No sentence fragments, no restating the
        // label ("Buffer size — the size of the buffer" teaches nothing).

        static let sampleRate = "Sample rate — measurements taken per second. Higher rates resolve faster changes and cost proportionally more storage."
        static let tolerance = "Tolerance — how close two values must be before they count as equal. Tightening it costs iterations; loosening it can hide a real difference."
        static let entryMode = "Native enters the parameters directly; Derived enters summary statistics and solves for the parameters."
        static let seed = "Seed — starting value for the random number generator. The same seed reproduces the same run exactly."

        /// Every entry above, by value. The lint checks this names all of
        /// them, because `isEntry(_:)` — and so the `?` affordance — depends
        /// on it being complete.
        static let all: Set<String> = [
            sampleRate, tolerance, entryMode, seed,
        ]
