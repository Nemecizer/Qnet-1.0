...
# Every header, footer and status band is a flat DS.Color.surface.
...
# `all` is the INDEX of the entries, not an entry: it is a Set<String>, no
# control ever passes it as `glossary:`, and a separate check below already
# proves it names every entry. Enumerating it here would demand that the
# index itself be wired to a control, which is not a thing.
done < <(grep -oE 'static let [A-Za-z0-9_]+' <<<"$glossary_block" | awk '{print $3}' | grep -vx 'all')
...
if [[ $status -ne 0 ]]; then
    echo
    echo "design_lint: violations found — see the adoption guide at the top of Sources/GUIKit/DesignSystem.swift"
fi
exit $status
