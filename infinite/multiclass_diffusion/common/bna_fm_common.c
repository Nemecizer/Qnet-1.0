...
int smart_solve(cholmod_sparse *A, double *b, double *x, cholmod_common *c) {
     ...
     * (semi-)definite.  CHOLMOD's supernodal Cholesky is by far the
     * fastest and most memory-efficient path for that structure:
     * typically 5-10× faster than UMFPACK's unsymmetric LU at our scales,
     * and the advantage grows with d (at d=4, n=10 the difference is the
     * gap between "unusable" and "a few seconds").
     *
     * The fallback chain protects against pathological numerics:
     *     CHOLMOD  →  UMFPACK  →  preconditioned CG
     */
    ...
    /* Direct solvers both failed — large-scale iterative solve. */
    if (!g_compact)
