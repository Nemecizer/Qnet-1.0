...
final class MetalTerminalRenderer: NSObject, MTKViewDelegate {
    ...
    private final class BufferPool {
        ...
        func makeBuffer<T>(_ vertices: [T]) -> MTLBuffer? {
            ...
            vertices.withUnsafeBytes { raw in
                // `_ =` so the closure returns Void: without it the closure
                // returns memcpy's pointer, withUnsafeBytes returns that, and
                // the unused result warns at every build.
    ...
    private static func candidateBundles() -> [Bundle] {
        ...
        // command-line executable (and an absolute build path). A signed macOS
        // app must keep resources under Contents/Resources, so support both
        // layouts without retaining a build-machine path at runtime.
        let packageBundleName = "SwiftTerm_SwiftTerm.bundle"
        let roots: [URL?] = [Bundle.main.resourceURL, Bundle.main.bundleURL]
        for root in roots.compactMap({ $0 }) {
            if let packageBundle = Bundle(
                url: root.appendingPathComponent(packageBundleName, isDirectory: true)
            ) {
